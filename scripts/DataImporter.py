...
for event in range(len(events['time_start'])):
    ...
    sat1.to_csv('../output-data/GSM/'+sat1_name+'/'+sat1_name+'_'+events['time_start'][event].strftime("%Y-%m-%d_%H-%M")+'.csv')
    sat2.to_csv('../output-data/GSM/'+sat2_name+'/'+sat2_name+'_'+events['time_start'][event].strftime("%Y-%m-%d_%H-%M")+'.csv')