...
def get_omni(start, stop, fixed):
    ...
    omni = {'Time': get_data('IMF')[0], 'BX_GSM': get_data('BX_GSE')[1], 'BY_GSM': get_data('BY_GSM')[1], 'BZ_GSM': get_data('BZ_GSM')[1], 'Vx': get_data('Vx')[1], 'Vy': get_data('Vy')[1], 'Vz': get_data('Vz')[1], 'proton_density': get_data('proton_density')[1], 'T': get_data('T')[1]}
    omni_data = pd.DataFrame(omni, columns=['Time', 'BX_GSM', 'BY_GSM', 'BZ_GSM', 'Vx', 'Vy', 'Vz', 'proton_density', 'T']).replace(to_replace=[999.990, 9999.99, 99999.9, 9999999], value=np.nan)
...
def get_artemis(start, stop, fixed):
    ...
    artemis_fgm_import = pyspedas.themis.fgm(trange=trange, probe='b', time_clip=True, varnames='thb_fgs_gsm')
    artemis_esa_import = pyspedas.themis.esa(trange=trange, probe='b', time_clip=True, varnames=['thb_peif_velocity_gsm', 'thb_peif_density', 'thb_peif_avgtemp'])
    artemis_stt_import = pyspedas.themis.state(trange=trange, probe='b', time_clip=True, varnames='thb_pos_gsm')

    #---ARTEMIS FGM data manipulation-------------------------------------------------------------------------------------------------------
    fgm = {'Time': get_data('thb_fgs_gsm')[0], 'BX_GSM': get_data('thb_fgs_gsm')[1][:,0], 'BY_GSM': get_data('thb_fgs_gsm')[1][:,1], 'BZ_GSM': get_data('thb_fgs_gsm')[1][:,2]}
    artemis_fgm_data = pd.DataFrame(fgm, columns=['Time', 'BX_GSM', 'BY_GSM', 'BZ_GSM']).replace(to_replace=[999.990, 9999.99, 99999.9], value=np.nan)
    ...
    esa = {'Time': get_data('thb_peif_velocity_gsm')[0], 'Vx': get_data('thb_peif_velocity_gsm')[1][:,0], 'Vy': get_data('thb_peif_velocity_gsm')[1][:,1], 'Vz': get_data('thb_peif_velocity_gsm')[1][:,2], 'proton_density': get_data('thb_peif_density')[1], 'T': get_data('thb_peif_avgtemp')[1]}
    ...
    state = {'Time': get_data('thb_pos_gsm')[0], 'Xpos': get_data('thb_pos_gsm')[1][:,0]}
...
def get_themis(start, stop, probe, fixed):
    ...
    trange = [start.strftime("%Y-%m-%d/%H:%M:%S"), stop.strftime("%Y-%m-%d/%H:%M:%S")] #Timeseries should be in the format [pd.Timestamp, pd.Timestamp]
    themis_fgm_import = pyspedas.themis.fgm(trange=trange, probe=probe, time_clip=True, varnames='th'+probe+'_fgs_gsm')
    themis_esa_import = pyspedas.themis.esa(trange=trange, probe=probe, time_clip=True, varnames=['th'+probe+'_peif_velocity_gsm', 'th'+probe+'_peif_density', 'th'+probe+'_peif_avgtemp'])

    #---THEMIS FGM data manipulation-------------------------------------------------------------------------------------------------------
    fgm = {'Time': get_data('th'+probe+'_fgs_gsm')[0], 'BX_GSM': get_data('th'+probe+'_fgs_gsm')[1][:,0], 'BY_GSM': get_data('th'+probe+'_fgs_gsm')[1][:,1], 'BZ_GSM': get_data('th'+probe+'_fgs_gsm')[1][:,2]}
    themis_fgm_data = pd.DataFrame(fgm, columns=['Time', 'BX_GSM', 'BY_GSM', 'BZ_GSM']).replace(to_replace=[999.990, 9999.99, 99999.9], value=np.nan)
    ...
    esa = {'Time': get_data('th'+probe+'_peif_velocity_gsm')[0], 'Vx': get_data('th'+probe+'_peif_velocity_gsm')[1][:,0], 'Vy': get_data('th'+probe+'_peif_velocity_gsm')[1][:,1], 'Vz': get_data('th'+probe+'_peif_velocity_gsm')[1][:,2], 'proton_density': get_data('th'+probe+'_peif_density')[1], 'T': get_data('th'+probe+'_peif_avgtemp')[1]}
